...
(function () {
    ...
    function protectHtml(protect, html) {
        each(protect, function (pattern) {
    ...
    function htmlToData(editor) {
        ...
        // Parse stylesheets
        data.stylesheets = [];
        each(headerFragment.getAll('link'), function (link) {
    ...
    function dataToHtml(editor, data) {
        ...
        each(headerFragment.getAll('link'), function (stylesheet) {
        ...
        // Add new
        each(data.stylesheets, function (stylesheet) {
        ...
        // Delete old
        each(currentStyleSheetsMap, function (stylesheet) {
            stylesheet.remove();
        });
    ...
    function setContent(editor, evt) {
        ...
        if (startPos != -1) {
            ...
            evt.content = tinymce.trim(content.substring(startPos + 1, endPos));
            foot = low(content.substring(endPos));
        } else {
            head = getDefaultHeader(editor);
            foot = '\n</body>\n</html>';
        }
        ...
        var currentStyleSheetsMap = {};
        each(headElm.getElementsByTagName('link'), function (stylesheet) {
    ...
    function getDefaultHeader(editor) {
        var header = '',
            value, styles = '';
    ...
    tinymce.create('tinymce.plugins.FullPagePlugin', {
        init: function (ed, url) {
            ...
            ed.onBeforeSetContent.add(function (ed, o) {
                o.selection = !ed.selection.isCollapsed();
                setContent(ed, o);
            });
        ...
        dataToHtml: function(data) {
            return dataToHtml(this.editor, data);
        }
    });
    // Register plugin
